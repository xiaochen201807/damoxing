/**
 * audit_risk_registry 模板专属参数分析脚本
 * 智能稽核 - 风险清册页面
 * 只保留1个API配置：列表，包含url和data
 */
const db = require('../db');
...
const API_PARAMS = {
    ...
    registry_list_api: {
        type: 'string',
        title: '风险清册列表接口 URL',
        description: '风险清册列表查询 API 地址',
        default: '/api/demo/audit/risk/registry/list',
        group: '🔗 列表接口配置',
        groupOrder: 1,
        order: 1
    },
...
// 主函数
async function analyzeAuditRiskRegistry() {
    const templateId = 'audit_risk_registry';
...
if (require.main === module) {
    analyzeAuditRiskRegistry().then(() => process.exit(0));
}

module.exports = analyzeAuditRiskRegistry;
