-- Oracle: JSON 方言 SQL 解析函数 + 业务函数/主过程完整脚本
-- 依赖：四张核心表、TMP_GJJ_YWBLSXZ、GJJ_YWBLBZ_LOG、T_WA_SYS_LOG_ERR、f_arg_list
-- 用途：覆盖现场已有 f_gjj_get_mxywsfz_sxz / p_gjj_get_mxywsfz，并补充 f_gjj_get_sql_by_dialect
...
v_errormsg varchar2(500);
  ...
  v_sxtjz  decimal(18,2);--通过属性判断得到的结果值
    v_sqlcx  varchar2(32767);
    v_sqltj  varchar2(32767);
    v_sqlsk  varchar2(32767);

    v_sqlsxz  varchar2(32767);
  v_gzmc  varchar2(2000);
    v_sxly  varchar2(32767);
  v_YWBLBZSX  varchar2(3000);
    v_ywblsql  varchar2(32767);
    v_sql_bz  varchar2(32767);
    v_ywbljgz  varchar2(32767);
  ...
  v_db_type varchar2(30):='oracle';----JSON 方言目标库类型
  ...
  if v_ywsf='1' THEN--最大可提取额
        ...
        open tmp_cur for v_sqlsk ;
          loop
            ...
            v_ywbljgz:=f_gjj_get_sql_by_dialect(v_ywbljgz,v_db_type);
            --------取值开始
                   ...
                   ELSE-------配置了详细参数配置
                        ...
                        open tmp_cur_sxz for v_sqlsxz ;
                             loop
                                    ...
                                    if v_sxly<>'page' then
                                      v_ywblsql:=f_gjj_get_sql_by_dialect(v_ywblsql,v_db_type);
                                    end if;
