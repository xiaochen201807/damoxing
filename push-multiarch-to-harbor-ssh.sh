...
SSH_TARGET="${SSH_TARGET:-xiaochen@127.0.0.1}"
SSH_HOST="${SSH_HOST:-}"
SSH_USER="${SSH_USER:-}"
SSH_PORT="${SSH_PORT:-2222}"
SSH_KEY="${SSH_KEY:-}"
SSH_EXTRA_ARGS="${SSH_EXTRA_ARGS:-}"
SSH_SOCKS_PORT="${SSH_SOCKS_PORT:-18180}"
...
# 长连接上传经过 SSH 隧道时可能偶发断流；留空可关闭默认重试。
SKOPEO_RETRY_TIMES="${SKOPEO_RETRY_TIMES:-5}"
...
run_skopeo_copy() {
    ...
    if [[ -n "$SKOPEO_RETRY_TIMES" && "$SKOPEO_EXTRA_ARGS" != *"--retry-times"* ]]; then
        skopeo_args+=(--retry-times "$SKOPEO_RETRY_TIMES")
    fi
