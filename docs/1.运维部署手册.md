...
本部分详细阐述了大模型项目的基础环境准备、架构规划及标准化的 Docker 部署流程。作为系统上线前的核心环节，要求运维人员必须严格遵循环境配置规范，确保服务器资源、操作系统及 Docker 环境满足最低运行要求。通过执行标准化的部署步骤，包括镜像加载、容器启动及安全配置，可确保系统快速、稳定地投入生产运行，同时为后续的运维管理奠定坚实基础。
...
├── docker-compose.yml     # 编排文件 (包含所有配置)
└── data/                  # [重要] 唯一的数据目录
...
# 1. 检查文件完整性
ls -la
# 应看到: damoxing.tar, docker-compose.yml, data/
...
> 💡 **提示**: 本项目采用**单文件配置**方式，直接修改 `docker-compose.yml` 即可，无需额外的 `.env` 文件，方便现场运维管理。
...
启动服务后，请执行以下检查以确保服务正常运行：

| 检查项 | 验证方法 | 预期结果 |
|--------|----------|----------|
| **容器状态** | `docker-compose ps` | 状态显示 `Up (healthy)` |
| **健康检查接口** | `curl http://localhost:3001/health` | 返回 `{"status":"ok",...}` |
| **前端页面** | 浏览器访问 `http://服务器IP:3001` | 显示登录页或首页 |
...
| 序号 | 项目 | 操作 | 说明 |
|------|------|------|------|
| 1 | **修改 JWT 密钥** | 编辑 `docker-compose.yml`，使用 `openssl rand -hex 32` 生成 | 默认密钥不安全 |
| 2 | **配置 CORS** | 编辑 `docker-compose.yml`，指定具体域名，不使用 `*` | 防止跨域攻击 |
...
1. 检查 `docker-compose.yml` 中的 `API_ROUTE_PREFIX` 是否与前端一致
